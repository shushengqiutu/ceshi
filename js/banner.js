...
function FragmentBanner(option) {
	...
	//切换下一张图片时触发的函数
	this.onChange = option.onChange?option.onChange:undefined;
	//收起本张图片时触发的函数
	this.onLeave = option.onLeave?option.onLeave:undefined;
	//完成初始化的函数
	this.onFinish = option.onFinish?option.onFinish:undefined;
		...
		for(var i = 0,iL = this.grid.line;i < iL;i++){
			...
			for(var j = 0,jL = this.grid.list;j < jL;j++){
				...
				this.setCss(newI,{
					...
					backgroundSize : 'cover',//this.size.width + 'px ' + this.size.height +'px',
